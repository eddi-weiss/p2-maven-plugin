...
public class P2Mojo extends AbstractMojo implements Contextualizable
{
   ...
   private void createFeature(P2FeatureDefinition p2featureDefinition)
   {
      try
      {
         List<P2Artifact> p2Artifacts = p2featureDefinition.getArtifacts();
         for (P2Artifact a : p2Artifacts)
         {
            a.getExcludes().addAll(p2featureDefinition.getExcludes());
         }
         Multimap<P2Artifact, ArtifactBundlerInstructions> bi = this.processArtifacts(p2Artifacts);
   ...
   private void executeP2PublisherPlugin() throws IOException, MojoExecutionException
   {
      ...
      BundlePublisher publisher = BundlePublisher.builder()
         ...
         .sourceDirectory(bundlesDestinationFolder.getParentFile().getAbsolutePath())
